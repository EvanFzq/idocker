<template>
    ...
    <div class="dashbord">
        ...
        <div><van-tag size="large" :color="statusColor">{{ statusLabel }}</van-tag></div>
        <van-circle v-model:current-rate="memoryCurrentRate" :rate="memoryRate" :speed="100">
            <span class="material-icons-outlined icon">
                keyboard
            </span>
            <div class="value memory">{{ fileSizeFormat(memoryUsage) || '-' }} / {{ fileSizeFormat(memoryLimit) || '-' }}
            </div>
            <div class="text">内存使用量</div>
        </van-circle>
    </div>
    <van-tabs v-model:active="activeTab">
        <van-tab class="tab" title="基本信息">
            <base-info :name="containerDetail.Name?.slice(1)" :image="containerDetail.Config?.Image"
                :start-time="containerDetail.State?.StartedAt" :create-time="containerDetail.Created"
                :cmd="containerDetail.Config?.Cmd" :entrypoint="containerDetail.Config?.Entrypoint" />
        </van-tab>
        <van-tab class="tab" title="挂载">
...
const memoryRate = computed(() => Number((memoryUsage.value * 100 / memoryLimit.value).toFixed(0)))

const containerDetail = ref<Partial<Container>>({})
const statusLabel = computed(() => containerDetail.value.State?.Status && statusLabelMap[containerDetail.value.State?.Status])
const statusColor = computed(() => containerDetail.value.State?.Status && statusColorMap[containerDetail.value.State?.Status])
